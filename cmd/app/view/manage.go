...
import (
	...
	"github.com/dtylman/pictures/manager/backuper"
	"github.com/dtylman/pictures/tasklog"
	"github.com/dtylman/pictures/manager/sourceremove"
)
...
func newManageView() *manageView {
	...
	mv.status = gowd.NewElement("div")
	mv.AddElement(mv.status)
...
func (bv *manageView) btnRemoveFromSrcClicked(sender *gowd.Element, event *gowd.EventElement) {
	...
	err:=sourceremove.Start(sourceFolder)
	if err != nil {
...
func (iv *manageView) updateProgress(status tasklog.Task) {
	iv.status.SetText(fmt.Sprintf("%v: %v",status.Name, status.Messages))
	if !status.Running {
